...
ssh-remote-choice-install-extension-desc = 安装 Zap 扩展以在此会话中启用文件浏览、代码评审和智能命令补全等智能体功能。
...
ai-document-save-and-sync-tooltip = 保存并将此计划自动同步到你的 Zap
ai-document-show-in-warp-drive = 在 Zap 中查看
...
ai-footer-enable-notifications-tooltip = 安装 Zap 插件以在 Zap 中启用丰富的智能体通知
ai-footer-notifications-setup-instructions = 通知设置说明
ai-footer-install-plugin-instructions-tooltip = 查看安装 Zap 插件的说明
ai-footer-update-warp-plugin = 更新 Zap 插件
ai-footer-plugin-update-available-tooltip = Zap 插件有新版本可用
ai-footer-plugin-update-instructions = 插件更新说明
ai-footer-plugin-update-instructions-tooltip = 查看更新 Zap 插件的说明
...
settings-ai-auto-show-rich-input-tooltip = 需要为你的编码智能体安装 Zap 插件
...
keybinding-desc-workspace-left-panel-warp-drive = 左侧面板：Zap
...
keybinding-desc-workspace-toggle-warp-drive = 切换 Zap
...
keybinding-desc-workspace-search-drive = 搜索 Zap
...
keybinding-desc-workspace-export-all-warp-drive-objects = 导出所有 Zap 对象
...
keybinding-desc-workspace-install-opencode-warp-plugin = [调试] 安装 OpenCode Zap 插件
keybinding-desc-workspace-use-local-opencode-warp-plugin = [调试] 使用本地 OpenCode Zap 插件（仅测试用）
...
drive-local-storage-limit-description = 本地存储上限会在此设备上生效。请移除不再需要的项目，为新的 Zap 对象腾出空间。
drive-sign-up = 本地管理
...
wasm-nux-object-kind-drive-objects = Zap 对象
...
auth-override-warning-confirm-title = 要删除个人 Zap 对象和偏好吗？
auth-override-warning-description = 看起来你刚刚通过网页浏览器登录了一个 Zap 账号。如果继续，当前匿名会话中的所有个人 Zap 对象和偏好都将被永久删除。
...
auth-logout-unsynced-drive-objects-warning = 你有 { $count } 个未同步的 Zap 对象。退出登录后将失去这些对象。
...
cli-agent-plugin-claude-install-title = 安装 Claude Code 的 Zap 插件
cli-agent-plugin-claude-install-subtitle = 确保你的机器已安装 jq，然后运行这些命令。
cli-agent-plugin-claude-add-marketplace-step = 添加 Zap 插件市场仓库
cli-agent-plugin-install-warp-plugin-step = 安装 Zap 插件
...
cli-agent-plugin-claude-update-title = 更新 Claude Code 的 Zap 插件
...
cli-agent-plugin-gemini-install-title = 安装 Gemini CLI 的 Zap 插件
cli-agent-plugin-gemini-run-command-restart = 运行以下命令，然后重启 Gemini CLI。
cli-agent-plugin-install-warp-extension-step = 安装 Zap 扩展
cli-agent-plugin-gemini-restart-note = 重启 Gemini CLI 以启用插件。
cli-agent-plugin-gemini-update-title = 更新 Gemini CLI 的 Zap 插件
cli-agent-plugin-update-warp-extension-step = 更新 Zap 扩展
cli-agent-plugin-gemini-restart-update-note = 重启 Gemini CLI 以启用更新。
cli-agent-plugin-opencode-install-title = 安装 OpenCode 的 Zap 插件
cli-agent-plugin-opencode-install-subtitle = 将 Zap 插件加入 OpenCode 配置，然后重启 OpenCode。
...
cli-agent-plugin-opencode-update-title = 更新 OpenCode 的 Zap 插件
...
agent-tip-add-context = 输入 `@` 将文件、块或 Zap 对象作为上下文添加到提示词。
...
onboarding-free-user-subscribe-item-drive = 不限量 Zap 对象和协作
...
auth-login-slide-title-warp-drive = 开始使用 Zap
...
auth-disable-warp-drive = 停用 Zap
auth-disable-ai-features = 停用 AI 功能
auth-enable-warp-drive = 启用 Zap
...
auth-disable-warp-drive-confirm-title = 确定要停用 Zap 吗？
auth-disable-ai-features-confirm-title = 确定要停用 AI 功能吗？
auth-disable-warp-drive-confirm-body = Zap 可让你跨设备保存工作流和知识，并与团队分享。继续后，你将无法使用以下功能：
...
auth-require-login-drive-limit = Zap 会将 Zap 对象保存在本机。
...
ai-footer-plugin-installed-restart-session = Zap 插件已安装。请重启会话以启用。
ai-footer-installing-warp-plugin = 正在安装 Zap 插件...
ai-footer-failed-install-warp-plugin = 安装 Zap 插件失败
ai-footer-plugin-updated-restart-session = Zap 插件已更新。请重启会话以启用。
ai-footer-updating-warp-plugin = 正在更新 Zap 插件...
ai-footer-failed-update-warp-plugin = 更新 Zap 插件失败
...
workspace-plan-synced-to-warp-drive-toast = 计划已同步到你的 Zap
