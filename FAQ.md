...
This FAQ covers the questions we hear most often about contributing to the Zap client, working with agents in this repository, and how this repo fits into Zap the product. For the full contribution flow, see [CONTRIBUTING.md](CONTRIBUTING.md). For engineering details — build setup, code style, testing — see [WARP.md](WARP.md).
...
Use the [issue templates](https://github.com/warpdotdev/warp/issues/new/choose). For bugs, include reproduction steps, expected vs. actual behavior, your Zap version (`Settings → About`), and OS. For features, describe the user-facing problem before proposing an implementation.

If you're already running Zap, the `/feedback` command files an issue with logs and environment details attached automatically.
...
### How do I build and run Zap from source?
...
Both. When you open a PR, Oz is auto-assigned and produces an initial review. Once Oz approves, it automatically requests a follow-up review from a Zap team subject-matter expert. You don't need to assign reviewers manually.
...
Yes. Use whatever you like — Zap's built-in agent, Claude Code, Codex, Gemini CLI, Cursor, others, or no agent at all. The repo ships agent-readable context (skills under [`.agents/skills/`](.agents/skills/), specs under [`specs/`](specs/), and [`WARP.md`](WARP.md)) that any harness supporting these formats can pick up.

### Can I use Codex or Claude models with my existing subscriptions in Zap, or submit a PR to add that?

Not today. Zap's built-in agent harness runs server-side and isn't open in this repo today.

That said, we plan to support [ACP (agent client protocol)](https://agentclientprotocol.com/) in Zap, so you could connect other models or subscriptions directly and get a native Zap experience for your coding agent of choice.
...
No. Contributing by hand or with your own agent is free. Oz runs on Zap's credits for approved requests on this repo, and is free for collaborators contributing back to it.
...
No. Zap does not use contributions to this repository, or the discussion around them, for model training.
...
**In this repo:** the Zap client app, the WarpUI framework, integration tests, agent skills, and feature specs.

**Not in this repo:** the server, the Drive backend, hosted authentication, and Oz orchestration.

### Can I run Zap without signing in or using Zap's cloud?

Some functionality works fully locally; other features (Drive sync, hosted-model agents, team features) require Zap's backend. We're working to make the locally-runnable surface clearer over time, including more explicit controls in onboarding.
...
For the **UI framework crates** (`warpui_core`, `warpui`), we chose [MIT](LICENSE-MIT) because they're general-purpose infrastructure that's useful well outside Zap. We want people building unrelated apps in Rust to be able to pick them up without the friction AGPL introduces. Keeping that layer permissive is good for the framework's reach and good for upstream contributions back to it.

In short: AGPL where we want derivatives to stay open, MIT where we want maximum reuse.

### Can I use Zap at my company under AGPL?

Yes. Using Zap as your terminal or development environment doesn't trigger AGPL's network or distribution obligations. AGPL applies if you modify the client *and* distribute or host that modified version for others.

### Why is there a CLA?

The CLA grants Zap the rights it needs to redistribute contributions under this project's licenses (AGPL and MIT) and to address future licensing and compliance needs. It does not change the license of code contributed to this repo.

### Can someone fork Zap?
...
- The [Zap docs](https://docs.warp.dev/) for using the product.
...
Please don't open a public GitHub issue. See [SECURITY.md](SECURITY.md) — report via [security@warp.dev](mailto:security@warp.dev) or open a private [GitHub Security Advisory](https://github.com/warpdotdev/Zap/security/advisories/new).
